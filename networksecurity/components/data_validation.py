...
class DataValidation:
    ...
    def validate_numerical_columns(self, dataframe:pd.DataFrame) -> bool:
        try:
            number_of_num_columns = len(self._schema_config["numerical_columns"])
            numerical_col = [col for col in dataframe.columns if dataframe[col].dtypes != 'o']
            if len(numerical_col) == number_of_num_columns:
                return True
            return False
        except Exception as e:
            raise NetworkSecurityException(e, sys)    
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not status:
                error_message = f"Train datframe does not contain all columns.\n"
            status = self.validate_number_of_column(dataframe=test_dataframe)
            if not status:
                error_message = f"Train datframe does not contain all columns.\n"

            # validate numerical of columns
                
            status = self.validate_numerical_columns(dataframe=train_dataframe)
            if not status:
                error_message = f"Train datframe does not contain numerical columns.\n"
